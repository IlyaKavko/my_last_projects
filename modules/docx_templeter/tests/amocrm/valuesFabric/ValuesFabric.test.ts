...
function defaultTest(dataSets: any): void {
    ...
    const aliases = aliasFabric.getAliases().filter(i => !isCrutch(i.field_name))
    const values = valuesFabric.getAllCurrentValues().filter(i => !isCrutch(i.field_name))
    chai.assert(values.length > available_fields.length)
    chai.assert(values.length === aliases.length)
}

function isCrutch(fieldName: string): boolean {
    switch (fieldName){
        case 'burs10day':
        case 'burs17day':
        case 'burs30day':
