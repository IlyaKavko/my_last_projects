...
export class Alias implements IAlias{
    ...
    protected readonly _field_type: string
    ...
    get isDate(): boolean {
        return this.field_type === 'date' || this.field_type === 'date_time'
    }
}
...
export class AliasFabric {
    ...
    private static createModelsFromModel(model) {
        ...
        if (model.field_type === 'numeric')
            ...
            return [model, AliasFabric.fetchInitialsModel(model)]
        else
            return [model]
    }
    ...
    private static fetchInitialsModel(model){
        const newModel = Object.assign({}, model)
        newModel.mode = 'in'
        return newModel
    }
